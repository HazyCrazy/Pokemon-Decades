...
class Pokemon
  ...
  # Modded Level Scaling (For precise levels and keeping a Pokemon's Evolution Stage static if holding the Eviolite item)
  def scale(new_level = nil)
    new_level = AutomaticLevelScaling.getScaledLevel if new_level.nil?
    max_level = pbGet(78) # Level Scale Constant (Seperate from a variable such as Gym Badges, for manual control. Used in Battle Mode. Default value is 100)
