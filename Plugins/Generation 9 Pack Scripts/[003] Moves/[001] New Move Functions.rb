...
class Battle::Move::IncreasePowerEachTimeHit < Battle::Move
  def pbBaseDamage(baseDmg, user, target)
    bonus = 25 * user.num_times_hit
    return [baseDmg + bonus, 200].min
  end
end
...
#===============================================================================
# Chilly Reception Overhaul (Now only lowers Speed)
#===============================================================================
# Decreases the target's Speed by 1 stage. Then, user
# switches out. Ignores trapping moves. (Parting Shot)
#-------------------------------------------------------------------------------
#class Battle::Move::LowerTargetSpeed1SwitchOutUser < Battle::Move::TargetMultiStatDownMove
class Battle::Move::SwitchOutUserStartHailWeather < Battle::Move::TargetMultiStatDownMove
  def initialize(battle, move)
    super
    @statDown = [:SPEED, 1]
  end
