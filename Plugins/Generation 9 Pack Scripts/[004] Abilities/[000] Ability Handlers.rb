...
Battle::AbilityEffects::OnSwitchIn.add(:NEUTRALIZINGGAS,
  proc { |ability, battler, battle, switch_in|
	...
	battle.pbDisplay(_INTL("Neutralizing gas is supressing all non-Form Changing Abilities, such as Slow Start and Intimidate!"))
...
Battle::AbilityEffects::OnEndOfUsingMove.add(:BATTLEBOND,
  proc { |ability, user, targets, move, battle|
	...
	battle.pbDisplay(_INTL("{1} powered up due to its drive to thrive!", user.pbThis))
    battle.pbHideAbilitySplash(user)
    showAnim = true
...
Battle::AbilityEffects::DamageCalcFromUser.add(:TRANSISTOR,
  proc { |ability, user, target, move, mults, power, type|
	...
	bonus = (Settings::MECHANICS_GENERATION >= 9) ? 1.5 : 1.5
