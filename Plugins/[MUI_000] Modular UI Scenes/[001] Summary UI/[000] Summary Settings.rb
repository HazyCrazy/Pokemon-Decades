...
class PokemonSummary_Scene
  ...
  def pbPageCustomOption(cmd)
	...
	if cmd == "Perfect IVs"
      GameData::Stat.each_main { |s| @pokemon.iv[:HP] = 31 }
	  GameData::Stat.each_main { |s| @pokemon.iv[:ATTACK] = 31 }
	  GameData::Stat.each_main { |s| @pokemon.iv[:DEFENSE] = 31 }
	  GameData::Stat.each_main { |s| @pokemon.iv[:SPECIAL_ATTACK] = 31 }
	  GameData::Stat.each_main { |s| @pokemon.iv[:SPECIAL_DEFENSE] = 31 }
	  GameData::Stat.each_main { |s| @pokemon.iv[:SPEED] = 31 }
      @pokemon.calc_stats
      return true
    end
	
	if cmd == "0 Speed IVs"
      GameData::Stat.each_main { |s| @pokemon.iv[:SPEED] = 0 }
      @pokemon.calc_stats
      return true
    end
	
	if cmd == "0 Attack IVs"
      GameData::Stat.each_main { |s| @pokemon.iv[:ATTACK] = 0 }
      @pokemon.calc_stats
      return true
    end
	
	if cmd == "0 Sp.Atk IVs"
      GameData::Stat.each_main { |s| @pokemon.iv[:SPECIAL_ATTACK] = 0 }
      @pokemon.calc_stats
      return true
    end
    #---------------------------------------------------------------------------
