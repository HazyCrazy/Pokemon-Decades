...
#===============================================================================
# Rocky Payload Super Yellow (Now places Stealth Rock on Contact Hit)
#===============================================================================
Battle::AbilityEffects::OnBeingHit.add(:ROCKYPAYLOAD,
  proc { |ability, user, target, move, battle|
    ...
    battle.pbAnimation(:STEALTHROCK, target, target.pbDirectOpposing)
    battle.pbDisplay(_INTL("Stellar stones were scattered all around {1}!", target.pbOpposingTeam(true)))
    battle.pbHideAbilitySplash(target)
  }
)

#===============================================================================
# Barbed Super Yellow (Iron Barbs, now sets a layer of Spikes on Contact Hit)
#===============================================================================
...
Battle::AbilityEffects::DamageCalcFromUser.add(:SHARPNESS,
  proc { |ability, user, target, move, mults, baseDmg, type|
    #mults[:power_multiplier] *= 1.5 if move.slicingMove?
	 mults[:power_multiplier] *= 1.3 if move.slicingMove?
  }
)
...
#===============================================================================
# Ice Age Rock (Purifying Salt Revamp, Rock Types resist Ghost types in SY, Rock/Ice Types only)
# 75% Damage Reduction to just Steel and Fighting has to be done, because Aurorus cannot function
# with it's x4 Steel and Fighting weaknesses. In pratice, this is balanced out by how Aurorus has 
# to dedicate an Ability slot to patch it's x4 weaknesses, that is directly responsible for killing it's viability.
# Eviolite Amaura is the most extreme this Ability becomes, tested without EV's and 31 IVs, all foes and players.
# Close Combat/Gigaton Hammer 1 to 3 times, will always defeat these two. 
#===============================================================================
Battle::AbilityEffects::DamageCalcFromTarget.add(:PURIFYINGSALT,
  proc { |ability, user, target, move, mults, baseDmg, type|
    mults[:attack_multiplier] /= 4 if type == :STEEL || :FIGHTING
  }
)
...
Battle::AbilityEffects::SpeedCalc.add(:PROTOSYNTHESIS,
  proc { |ability, battler, mult, ret|
	...
	next mult * 1.3 if battler.effects[PBEffects::ParadoxStat] == :SPEED
  }
)
